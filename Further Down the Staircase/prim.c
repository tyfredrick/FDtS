	...
	case WM_PAINT: {
		...
		if(mode == PLAY){
			...
			while(occupied != NULL){
				...
				FillRect(device_context, &sillySquare, CreateSolidBrush(RGB(r, g, b)));
				char sqrString[20];
			...
			if(mode == SELECT){
				...
				FillRect(device_context, &selectRect, CreateSolidBrush(RGB(*(cs->selectorRGB), *(cs->selectorRGB + 1), *(cs->selectorRGB + 2))));
				for(int a = 0; a < columnNo; a++){
					for(int i = 0; i < cs->columnSize; i++){
						...
						FillRect(device_context, &tempRect, CreateSolidBrush(RGB(*(cs->optRGB), *(cs->optRGB + 1), *(cs->optRGB + 2))));
					}
				}
